...
\title{WISE: Connectivity-Sustaining Equilibrium Selection\\
for Heterogeneous Multi-Robot Transport}
...
active face, cast relaxed WISE existence and selection as a semidefinite program, and
give a sufficient small-gain condition relating the connectivity requirement to
exponential stability of the coupled served-aggregate/estimator subsystem. A
lexicographic selector preserves the productive optimum exactly while maximizing
connectivity. In the evaluated instances, compositions with identical productive value
and wrench feasibility span algebraic connectivities from $\approx0$ (disconnected) to
$0.78$, while the relaxed WISE SDP certifies all $30$ paired cases; a single
randomized-rounding draw re-certifies $66\%$ of the $30$ seeds and up to $30$ draws
re-certify all of them. The study separates wrench from communication infeasibility and
quantifies the heuristic attainment gap.
\end{abstract}
