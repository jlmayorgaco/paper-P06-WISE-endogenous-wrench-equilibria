...
Laplacian} $\Lap(z)$ depends only on the assignment $z$ over $V$ candidate-site
nodes. A fixed base graph $L_0\succeq0$ carries the always-on intra-cluster links;
...
(Section~\ref{sec:algorithm}). The requirement $\sigmastar=\sigmadyn+\delta$ pairs the
...
\delta_{\mathrm{num}}$ documented in the generated certificate.