...
\end{theorem}
\begin{proof}[Proof sketch]
$\Gamma_{\Emanifold}\ge0$ since $d=0$ is admissible. By concavity of $\lamtwo\circ\Lap$,
...
means $B\hat z=y^{\star}$, declared numerically when
$\|B\hat z-y^{\star}\|_\infty\le\tau_B=10^{-8}$; an integer self-sustaining assignment need not
be zero-cost. Every WISE \emph{is} a productive variational equilibrium: connectivity acts as a
...
optimal multiplier set~\cite{boyd2004convex}. The cases are $\sigma\le\LamE$, $\sigma>\LamX$,
and between.
\end{proof}

\subsection{Finite-team recovery}
A sufficient margin test preserves \emph{joint feasibility} after integer recovery (not the
aggregate). With $z^{\star}$ a relaxed WISE, define slacks and sensitivities
\begin{equation}
\begin{aligned}
...
guarantee: best-of-$R$ rounding \emph{searches} for a feasible $\hat z$ and re-checks it
directly against budgets, occupancy, the lifted wrench LP and $\lamtwo\ge\sigmastar$. The
radius~\eqref{eq:intmargin} is a separate worst-case certificate dominated by
$m_\lambda/\kappa_L$ (with $\kappa_L$ itself bounded conservatively), and
Section~\ref{sec:exp} shows it certifies almost nothing---direct re-certification is the
operative finite-team test.
\end{remark}
